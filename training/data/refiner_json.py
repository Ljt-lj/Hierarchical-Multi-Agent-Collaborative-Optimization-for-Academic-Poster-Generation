"""Refiner 标签 → 可解析 ContentNode JSON（build_sft / evaluate 共用）."""
...
def p2p_dict_to_content_tree(obj: dict[str, Any]) -> dict[str, Any]:
    if CONTENT_KEYS.intersection(obj.keys()):
        return {
            ...
            "children": [p2p_dict_to_content_tree(c) for c in (obj.get("children") or []) if isinstance(c, dict)],
        }
...
def is_parseable_content_json(text: str) -> bool:
    try:
        obj = json.loads(balanced_json_slice(text) or text)
        ContentNode.from_dict(obj)
        return True
    except Exception:
        try:
